...
A_hat = zeros( m, n);
E_hat = zeros( m, n);
mu = 1.25/norm_two; % this one can be tuned % Max Apr 9: suppressed output
mu_bar = mu * 1e7;  % Max Apr 9: suppressed output
rho = 1.5;          % this one can be tuned % Max Apr 9: suppressed output
d_norm = norm(D, 'fro');
...
while ~converged       
    ...
    if mod( total_svd, 10) == 0 % Max Apr 9
